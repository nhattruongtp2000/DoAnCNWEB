...
namespace WebAPI.ApiIntegration
{
    public class SizeApiClient : BaseApiClient, ISizeApiClient
    {
        ...
        {
            return await GetListAsync<SizeVm>("/api/sizes?languageId=" + languageId);
        }

        public async  Task<SizeVm> GetById(string id, string languageId)
        {
            var data = await GetAsync<SizeVm>($"/api/sizes/{id}/{languageId}");
        ...
        public async Task<PagedResult<SizeVm>> GetSizesPagings(GetSizePagingRequest request)
        {
            var data = await GetAsync<PagedResult<SizeVm>>(
                ...
                $"&keyword={request.Keyword}&languageId={request.LanguageId}&categoryId={request.SizeId}");
